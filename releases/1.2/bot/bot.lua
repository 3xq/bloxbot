...
while not game:IsLoaded() or not game:GetService("CoreGui") or not game:GetService("Players").LocalPlayer or not game:GetService("Players").LocalPlayer.PlayerGui or not game:GetService("Players").LocalPlayer.Character or not game:GetService("Players").LocalPlayer.Character.HumanoidRootPart do wait() end
...
local function is_whitelisted(user_id)
    for _, data in pairs(whitelist) do
        if data.roblox == user_id then
            return true
        end
    end
...
success, error_message = pcall(function()
    for _, player in pairs(players:GetPlayers()) do
        ...
        if player_character and player_humanoidrootpart then 
            local whitelist = json_decode(readfile('whitelist.json'))

            humanoidrootpart.CFrame = player_humanoidrootpart.CFrame
            chat('.gg/EdgeN35M3D | come get whitelisted, the bots wont attacked you anymore | come chill, i dont fling people | mr. doinboin on top')
